...
public abstract class AbstractContext<A,I,O extends FCAObject<A,I>> implements Context<A,I,O> {
	 ...
	 */
	private IndexedSet<A> attributes = new ListSet<A>();
	 ...
	 * @param index index of the attribute whose name is requested
	 * @return name of the attribute at index <code>index</code>
	 */
	public A getAttributeAtIndex(int index) {
		return attributes.getElementAt(index);
	}
	...
	/** 
	 * Adds the attributes in a given set to the set of attributes of this context.
	 * @param attrs the set of attributes to be added
	 * @return <code>true</code> if all of the <code>attributes</code> are successfully
	 * added
	 * @throws IllegalAttributeException if one of the attributes in <code>attributes</code>
	 * already occurs in the attributes of this context
	 */
	 ...
	 * @return <code>true</code> if the <code>object</code> is successfully added
	 * @throws IllegalObjectException if an object with the same name already exists in this
	 * context
	 */
	 ...
	 */
	public abstract boolean removeObject(O object) throws IllegalObjectException;
	...
	/** 
	 * Adds the objects in a given set of objects to the set of objects of this context.
	 * @param s the set of objects to be added
	 * @return <code>false</code> if at least one of the <code>objects</code> is not successfully
	 * added
	 */
	 ...
	 */
	public abstract boolean isCounterExampleValid(O counterExample, FCAImplication<A> imp);
	...
	//  * confirmed implication to the base. Extensions of GenericContext class can override it to
	//  * perform extra operations like for instance for a DL context add the implication to the knowledge base
	//  * as a GCI.
	...
	// /**
	//  * Performs the necessary operations when the expert provides a counterexample. For instance, check
	//  * whether it is valid, and so on.
	//  * @param counterExample the counterexample that the expert has provided
	//  * @param question the question that has been rejected by the expert, and has resulted in
	//  * <code>counterExample</code>
	//  */
	// public synchronized void counterExampleProvided(O counterExample, FCAImplication<A> question) {
	...
	// 			expert.counterExampleInvalid(counterExample, Expert.COUNTEREXAMPLE_EXISTS);
	// 			expert.requestCounterExample(question);
	// 		}
	// 	}
	// 	else {
	...
	/**
	 * Checks whether an implication already follows from some background knowledge, in case it
	 * exists. For instance, in a DL context it would check if a  given implication (in this case 
	 * a subsumption relation) already follows from the DL knowledge base.
	 * @param implication
	 * @return <code>true</code> if <code>implication</code> already follows from background
	 * knowledge, <code>false</code> otherwise
	 */
	protected abstract boolean followsFromBackgroundKnowledge(FCAImplication<A> implication);
	
	// /**
	//  * Starts attribute exploration with empty premise.
	//  * @throws IllegalExpertException if the expert has not been set before starting exploration
	//  */
	...
	// /**
	//  * Resets attribute exploration. Clears the set of implications, the set of objects, the set of
	//  * attributes and the premise. After a call to this method, this context is ready for a fresh exploration.
	//  */
	 ...
	 * new set of implications. If not, the expert is requested to answer the implication question.
	 * Extensions of this class should implement the abstract method 
	 * {@link #followsFromBackgroundKnowledge(FCAImplication)} for their specific purposes.
	 */
